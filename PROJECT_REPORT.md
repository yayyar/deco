...
│   ├── pos/
│   │   ├── CheckoutDialog.kt           # Payment modal (Cash, KPay, WavePay, Custom methods, Split, Discounts) with horizontal method row
...
│   │   ├── ReceiptPreviewDialog.kt     # Full-fidelity thermal receipt voucher preview with dynamic Print & Share actions
│   │   └── ReceiptSuccessDialog.kt     # Post-checkout modal with slip preview trigger
│   └── settings/
...
#### 5.6.1. Full-Fidelity Receipt Slip Preview Flow
Before triggering printing or sharing from the sale completion screen (`ReceiptSuccessDialog`), the application seamlessly routes the user through `ReceiptPreviewDialog`:
- **Live Thermal Slip Rendering**: Renders the complete, pixel-accurate receipt bitmap generated by `ReceiptBitmapRenderer` inside a scrollable thermal paper card.
- **Contextual Intent Handling**: Dynamically configures the primary filled action button based on the user's initiating click:
  - If opened from **Share Slip**: **Share Slip** is emphasized as primary with **Print** as secondary.
  - If opened from **Print**: **Print Receipt** is emphasized as primary with **Share** as secondary.
- **Bilingual Title & Controls**: Complete localized header and dismiss actions for English and Burmese.
...
4. **Dynamic Myanmar Payment Matrix with Horizontal Selection**:
   ...
   - Clean horizontal scrollable chip row (`PaymentMethodRow` via `LazyRow`) in `CheckoutDialog` for fast, ergonomic one-tap channel selection.
...
6. **Voucher Slip Preview & Social Sharing**:
   - Full-fidelity receipt slip preview modal (`ReceiptPreviewDialog`) enabling visual verification prior to ESC/POS thermal printing or instant image dispatch to Viber, Messenger, Telegram, or WhatsApp.
7. **Draft / Parked Sales Workflow**:
   - Instant parking and restoring of open orders with active notification badges in the navigation bar.
8. **Real-time Low Stock Guard**:
   - Threshold-based visual warnings when inventory dips below safe operational levels.
9. **Comprehensive Settings & Hardware Setup**:
